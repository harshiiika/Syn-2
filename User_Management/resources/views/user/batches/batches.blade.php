        ...
        <table class="table table-hover" id="table">
          ...
          @foreach($batches as $index => $batch)
            <tr>
              ...
              <td>{{ $batch->shift ?? '—' }}</td>
              <td>
                <span class="badge {{ $batch->status === 'Not Assigned' ? 'bg-danger' : 'bg-success' }}">
                  {{ $batch->status ?? 'Not Assigned' }}
                </span>
              </td>
              <td>
                <div class="dropdown">
                  ...
                  <ul class="dropdown-menu" aria-labelledby="actionMenuButton">
                    <li>
                      <form method="POST" action="{{ route('batches.toggleStatus', $batch->id) }}">
                        @csrf
                        <button type="submit" class="dropdown-item">
                          {{ $batch->status === 'Assigned' ? 'Unassign' : 'Assign' }}
                        </button>
                      </form>
                    </li>
                  </ul>
...
<script src="{{asset('js/batchesa.js')}}"></script>
